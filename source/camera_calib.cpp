    ...
    for (int round = 0; round < 3; round++) {
      for (size_t a = 0; a < calibra.cams.size(); a++) {
        ...
        for (int iter = 0; iter < max_iter; iter++) {
          ...
          adjust_euler[round] = fix_adjust_euler[round] + pow(-1, iter) *
                                                              int(iter / 2) *
                                                              search_resolution;
          ...
          calibra.buildVPnp(calibra.cams[a], test_params, match_dis, false,
                            calibra.cams[a].rgb_edge_clouds,
          ...
          if (cost < min_cost) {
            ...
            calibra.buildVPnp(calibra.cams[a], test_params, match_dis, true,
                              calibra.cams[a].rgb_edge_clouds,
                              calibra.lidar_edge_clouds, pnp_list);
            // cv::Mat projection_img = calibra.getProjectionImg(test_params, a,
            // 0); std::string img_name = std::to_string(a) + "_projection";
