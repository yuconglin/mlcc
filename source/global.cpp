...
int main(int argc, char** argv) {
  ...
  ros::Publisher pub_surf1 =
      nh.advertise<sensor_msgs::PointCloud2>("/map_surf1", 100);
  ros::Publisher pub_surf2 =
      nh.advertise<sensor_msgs::PointCloud2>("/map_surf2", 100);
  ros::Publisher pub_surf3 =
      nh.advertise<sensor_msgs::PointCloud2>("/map_surf3", 100);

  string data_path, log_path;
  int max_iter, base_lidar, ref_lidar1, ref_lidar2, ref_lidar3 = 0;
  ...
  nh.getParam("ref_lidar1", ref_lidar1);
  ...
  for (size_t i = 0; i < ref_size; i++) {
    for (size_t j = 0; j < pose_size; j++) {
      ...
      pcl::io::loadPCDFile(
          data_path + to_string(ref_lidar[i]) + "/" + to_string(j) + ".pcd",
          *pc_ref);
  ...
  for (size_t i = 0; i < pose_size; i++) {
...

    {
      int j = 0;
      pcl::io::loadPCDFile(
          data_path + to_string(ref_lidar[j]) + "/" + to_string(i) + ".pcd",
          *pc_surf);
      mypcl::transform_pointcloud(
          *pc_surf, *pc_surf,
          q0.inverse() * (pose_vec[i].t - t0) +
              q0.inverse() * pose_vec[i].q * ref_vec[j].t,
