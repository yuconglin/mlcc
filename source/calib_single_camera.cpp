...
class vpnp_calib {
 public:
  ...
  bool operator()(const T *_q, const T *_t, T *residuals) const {
    ...
    T r4 = r2 * r2;
    T distortion =
        1.0 + distorT[0] * r2 + distorT[1] * r4 + distorT[4] * r2 * r4;
...
class vpnp_calib_rotation {
 public:
  vpnp_calib_rotation(VPnPData p, Vector3d trans_) {
    pd = p;
    trans = trans_;
  }
  template <typename T>
  ...
  static ceres::CostFunction *Create(VPnPData p, Vector3d trans_) {
    return (new ceres::AutoDiffCostFunction<vpnp_calib_rotation, 2, 4>(
        new vpnp_calib_rotation(p, trans_)));
  }
  ...
  for (int n = 0; n < 2; n++)
    for (int round = 0; round < 3; round++) {
      ...
      for (int iter = 0; iter < max_iter; iter++) {
        ...
        calibra.buildVPnp(calibra.camera_, test_params, match_dis, false,
                          calibra.camera_.rgb_edge_cloud_,
        ...
        if (cost < min_cost) {
          ...
          calibra.buildVPnp(calibra.camera_, test_params, match_dis, true,
                            calibra.camera_.rgb_edge_cloud_,
                            calibra.lidar_edge_cloud_, pnp_list);
          cv::Mat projection_img = calibra.getProjectionImg(test_params);
          cv::resize(projection_img, projection_img, cv::Size(), 0.5, 0.5,
                     cv::INTER_LINEAR);
...
int main(int argc, char **argv) {
  ...
  for (int dis_threshold = 30; dis_threshold > dis_thr_low_bound;
       dis_threshold -= 1) {
    LOG(INFO) << "Iteration:" << iter++ << " Distance:" << dis_threshold;
    for (int cnt = 0; cnt < 2; cnt++) {
      Eigen::Vector3d euler_angle =
          calib.camera_.ext_R.eulerAngles(2, 1, 0);  // 2 is z, 0 is x
      ...
      inner << calib.camera_.fx_, calib.camera_.s_, calib.camera_.cx_, 0,
          calib.camera_.fy_, calib.camera_.cy_, 0, 0, 1;
#ifdef FISHEYE
      distor << calib.camera_.k1_, calib.camera_.k2_, calib.camera_.k3_,
          calib.camera_.k4_;
#else
      distor << calib.camera_.k1_, calib.camera_.k2_, calib.camera_.p1_,
          calib.camera_.p2_, calib.camera_.k3_;
#endif

      calib.buildVPnp(calib.camera_, calib_params, dis_threshold, true,
                      calib.camera_.rgb_edge_cloud_, calib.lidar_edge_cloud_,
                      vpnp_list);
  ...
  calib_params << euler_angle(0), euler_angle(1), euler_angle(2), transation(0),
      transation(1), transation(2);
  calib.colorCloud(calib_params, 1, calib.camera_, calib.camera_.rgb_img_,
                   calib.lidar_cloud_);
