...
class Calibration {
 public:
  ...
  ros::Publisher pub_residual =
      _nh.advertise<sensor_msgs::PointCloud2>("/residual", 1000);
  ros::Publisher pub_direct =
      _nh.advertise<visualization_msgs::MarkerArray>("/direct", 1000);
  ...
  Calibration(const std::string& CamCfgPaths, const std::string& CalibCfgFile,
              bool use_ada_voxel) {
    ...
    // camera_.dist_coeffs_, camera_.rgb_img_.size(),
    ...
    edgeDetector(rgb_canny_threshold_, rgb_edge_minLen_, gray_img, rgb_edge_img,
                 camera_.rgb_edge_cloud_);
  ...
  void loadCameraConfig(const std::string& CamCfgPaths) {
    ...
    camera_.ext_R << camera_.init_ext_.at<double>(0, 0),
        camera_.init_ext_.at<double>(0, 1), camera_.init_ext_.at<double>(0, 2),
        camera_.init_ext_.at<double>(1, 0), camera_.init_ext_.at<double>(1, 1),
        camera_.init_ext_.at<double>(1, 2), camera_.init_ext_.at<double>(2, 0),
  ...
  void loadImgAndPointcloud() {
    lidar_cloud_ = pcl::PointCloud<pcl::PointXYZI>::Ptr(
        new pcl::PointCloud<pcl::PointXYZI>);
    pcl::io::loadPCDFile(data_path_ + std::to_string(lidar_number_) + ".pcd",
                         *lidar_cloud_);
    ROS_INFO_STREAM("Sucessfully load Point Cloud");
    camera_.rgb_img_ = cv::imread(
        data_path_ + std::to_string(image_number_) + ".png", cv::IMREAD_COLOR);
    ROS_INFO_STREAM("Sucessfully load Image");
  }

  void cut_voxel(unordered_map<VOXEL_LOC, OCTO_TREE_ROOT*>& feat_map,
                 pcl::PointCloud<pcl::PointXYZI>& pl_feat, double voxel_size,
                 float eigen_ratio) {
    float loc_xyz[3];
  ...
  void estimate_edge(std::unordered_map<VOXEL_LOC, OCTO_TREE_ROOT*>& surf_map) {
    ros::Rate loop(500);
    lidar_edge_cloud_ = pcl::PointCloud<pcl::PointXYZI>::Ptr(
        new pcl::PointCloud<pcl::PointXYZI>);
    for (auto iter = surf_map.begin(); iter != surf_map.end(); iter++) {
  ...
  void buildVPnp(
    ...
    for (size_t i = 0; i < lidar_edge_clouds_3d->size(); i++) {
      ...
      if (cos_angle(q_ * pt1 + t_, pt2) >
          cos(DEG2RAD(cam_fov_ / 2.0)))  // fisheye cam FoV check
        pts_3d.emplace_back(cv::Point3f(pt1(0), pt1(1), pt1(2)));
    }
#ifdef FISHEYE
    cv::fisheye::projectPoints(pts_3d, pts_2d, r_vec, t_vec,
                               camera_.camera_matrix_, camera_.dist_coeffs_);
#else
    ...
    if (show_residual) {
      cv::Mat residual_img = getConnectImg(
          cam, dis_threshold, cam_edge_clouds_2d, line_edge_cloud_2d);
      cv::resize(residual_img, residual_img, cv::Size(), 0.5, 0.5,
                 cv::INTER_LINEAR);
  ...
  void colorCloud(const Vector6d& extrinsic_params, const int density,
                  const Camera& cam, const cv::Mat& rgb_img,
    ...
    for (size_t i = 0; i < lidar_cloud->size(); i += density) {
      ...
      Eigen::Vector3d pt2(0, 0, 1);
      if (cos_angle(q_ * pt1 + t_, pt2) >
          cos(DEG2RAD(cam_fov_ / 2.0)))  // FoV check
      {
