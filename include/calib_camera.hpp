...
class Calibration {
 public:
  ...
  int total_ext_number;
  int base_number;
  std::vector<mypcl::pose> base_poses, extrinsics;
  // base LiDAR clouds local frame
  std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr> base_clouds;
  std::vector<LiDAR> ext_lidars;
  // 存储平面交接点云 global frame
  ...
  float cos_direction_theta_min_;
  float cos_direction_theta_max_;
  ...
  Calibration(const std::vector<std::string>& CamCfgPaths,
    ...
    if (use_ada_voxel) {
      ...
      // Voxelize the base_cloud.
      cut_voxel(surf_map, voxel_size_, eigen_ratio_);

      // Cut the voxels till there are too few points in it or it is a plane
      // voxel.
      LOG(INFO) << "............ cutting the voxels ..........";
      for (auto iter = surf_map.begin(); iter != surf_map.end(); ++iter) {
        iter->second->recut();
      }
      // Extract the edges from LiDAR point clouds.
      LOG(INFO) << "........... extracting edges from point clouds.........";
      ...
      // We are not using this block now.
    ...
    for (size_t i = 0; i < cams.size(); ++i) {
      for (size_t j = 0; j < cams[i].rgb_imgs.size(); ++j) {
        if (!cams[i].rgb_imgs[j].data) {
          const std::string msg = "Can not load image from " + lidar_path;
    ...
    for (size_t i = 0; i < cams.size(); ++i) {
      std::vector<cv::Mat> grey_imgs;
      std::vector<cv::Mat> rgb_edge_imgs;
  ...
  bool loadCalibConfig(const std::string& config_file,
    ...
    const double normal_theta_min_deg = fSettings["Plane.normal_theta_min"];
    const double normal_theta_max_deg = fSettings["Plane.normal_theta_max"];
    cos_normal_theta_min_ = cos(DEG2RAD(normal_theta_min_deg));
    cos_normal_theta_max_ = cos(DEG2RAD(normal_theta_max_deg));

    const double direction_theta_min_deg =
        fSettings["Edge.direction_theta_min"];
    const double direction_theta_max_deg =
        fSettings["Edge.direction_theta_max"];
    cos_direction_theta_min_ = cos(DEG2RAD(direction_theta_min_deg));
    cos_direction_theta_max_ = cos(DEG2RAD(direction_theta_max_deg));
  ...
  void cut_voxel(unordered_map<VOXEL_LOC, OCTO_TREE_ROOT*>& feat_map,
    ...
    LOG(INFO) << "............ Point cloud voxelized..........";
  }
  ...
  void estimate_edge(std::unordered_map<VOXEL_LOC, OCTO_TREE_ROOT*>& surf_map) {
    ...
    for (auto iter = surf_map.begin(); iter != surf_map.end(); iter++) {
      ...
      for (const auto& pv : iter->second->all_points) {
        pcl::PointXYZI p;
        p.x = pv(0);
        p.y = pv(1);
        p.z = pv(2);
      ...
      mergePlane2(plane_list, merge_plane_list);

      if (merge_plane_list.size() <= 1) {
        continue;
      }

      // publishes extracted planes from point clouds.
      for (const auto& plane : merge_plane_list) {
        ...
        for (const auto& pv : plane->plane_points) {
          pcl::PointXYZRGB pi;
          pi.x = pv[0];
          pi.y = pv[1];
          pi.z = pv[2];
        ...
        for (size_t p2_index = p1_index + 1; p2_index < merge_plane_list.size();
          ...
          for (size_t j = 0; j < line_point.size(); j++) {
            ...
            if (kd_tree.nearestKSearch(p, K, pointIdxNKNSearch,
              ...
              // The distance to the fartherest point in the K nearest neighbour
              // is less than 0.05.
  ...
  void mergePlane2(std::vector<Plane*>& origin_list,
                   std::vector<Plane*>& merge_list) {
    const int n = origin_list.size();
    UnionFindSet ufs(n);

    const double max_cos_diff = std::cos(5. / 180 * M_PI);

    for (int i = 0; i < n; ++i) {
      const auto* plane_i = origin_list[i];
      for (int j = i + 1; j < n; ++j) {
        const auto* plane_j = origin_list[j];
        // Normal difference.
        if (plane_i->normal.dot(plane_j->normal) < max_cos_diff) {
          continue;
        }
        /*
        const Eigen::Vector3d normal_diff = plane_i->normal - plane_j->normal;
        const Eigen::Vector3d normal_add = plane_i->normal - plane_j->normal;
        if (normal_diff.norm() >= 0.2 || normal_add.norm() >= 0.2) {
          continue;
        }
        */
        // Flatness
        const double dis1 = plane_i->normal.dot(plane_j->center) + plane_i->d;
        const double dis2 = plane_j->normal.dot(plane_i->center) + plane_j->d;
        if (dis1 < 0.05 && dis2 < 0.05) {
          ufs.Union(i, j);
        }
      }
    ...
    // Merge planes with the same root.
    std::map<int, std::vector<int>> root_and_members;
    for (int i = 0; i < n; ++i) {
      root_and_members[ufs.Find(i)].push_back(i);
    }

    for (const auto& [_, ids] : root_and_members) {
      Plane* merge_plane = new Plane;
      for (int id : ids) {
        merge_plane->plane_points.insert(merge_plane->plane_points.end(),
                                         origin_list[id]->plane_points.begin(),
                                         origin_list[id]->plane_points.end());
      }

      merge_plane->points_size = merge_plane->plane_points.size();
      for (const auto& pv : merge_plane->plane_points) {
        merge_plane->covariance += pv * pv.transpose();
        merge_plane->center += pv;
      }
      ...
      Eigen::EigenSolver<Eigen::Matrix3d> es(merge_plane->covariance);
      const Eigen::Matrix3d& evecsReal = es.eigenvectors().real();
      const Eigen::Vector3d& evalsReal = es.eigenvalues().real();
      Eigen::Matrix3f::Index evalsMin;
      Eigen::Matrix3f::Index evalsMax;
      evalsReal.rowwise().sum().minCoeff(&evalsMin);
      evalsReal.rowwise().sum().maxCoeff(&evalsMax);

      merge_plane->normal =
          Eigen::Vector3d(evecsReal(0, evalsMin), evecsReal(1, evalsMin),
                          evecsReal(2, evalsMin));
      merge_plane->min_eigen_value = evalsReal(evalsMin);
      merge_plane->radius = std::sqrt(evalsReal(evalsMax));
      merge_plane->d = -merge_plane->normal.dot(merge_plane->center);

      merge_plane->p_center.x = merge_plane->center.x();
      merge_plane->p_center.y = merge_plane->center.y();
      merge_plane->p_center.z = merge_plane->center.z();

      merge_plane->p_center.normal_x = merge_plane->normal.x();
      merge_plane->p_center.normal_y = merge_plane->normal.y();
      merge_plane->p_center.normal_z = merge_plane->normal.z();
      merge_plane->is_plane = true;
  ...
  void projectLine(const Plane* plane1, const Plane* plane2,
                   std::vector<Eigen::Vector3d>& line_point) {
    const float cos_normal_theta = plane1->normal.dot(plane2->normal);
    if (!(cos_normal_theta > cos_normal_theta_max_ &&
          cos_normal_theta < cos_normal_theta_min_)) {
      return;
    }

    const Eigen::Vector3d& c1 = plane1->center;
    const Eigen::Vector3d& c2 = plane2->center;
    const Eigen::Vector3d& n1 = plane1->normal;
    const Eigen::Vector3d& n2 = plane2->normal;
    ...
    // It says :
    // n1 * (x - c1) = 0 (point on plane 1)
    // (n1 x n2) * (x - c1) = 0 (the line from c1 to point is perpendicular to
    // n1 x n2.)
    // n2 * (x - c2) = 0 (point on plane 2)
    Eigen::Vector3d O = A.colPivHouseholderQr().solve(b);
    // distance from c1 to the point.
    const double c1_to_line = (c1 - O).norm();
    // distance from c2 to the point.
    const double c2_to_line = ((c2 - O) - (c2 - O).dot(d) * d).norm();
    ...
    if (plane1->points_size < plane2->points_size) {
      for (const auto& pt : plane1->plane_points) {
        // The projected point along the intersection line.
        const Eigen::Vector3d p = (pt - O).dot(d) * d + O;
        line_point.push_back(p);
      }
    } else {
      for (const auto& pt : plane2->plane_points) {
        // The projected point along the intersection line.
        const Eigen::Vector3d p = (pt - O).dot(d) * d + O;
        line_point.push_back(p);
      }
  ...
  void edgeDetector(
      int canny_threshold, int edge_threshold,
  ...
  void buildVPnp(
      const Camera& cam, const Vector6d& extrinsic_params, int dis_threshold,
      bool show_residual,
    ...
    const cv::Mat camera_matrix =
        (cv::Mat_<double>(3, 3) << cam.fx_, cam.s_, cam.cx_, 0.0, cam.fy_,
         cam.cy_, 0.0, 0.0, 1.0);
    const cv::Mat distortion_coeff =
        (cv::Mat_<double>(1, 5) << cam.k1_, cam.k2_, cam.p1_, cam.p2_, cam.k3_);

    // We can't use "const xxx =" here due to Eigen's API.
    ...
    for (size_t a = 0; a < base_poses.size(); ++a) {
      // for each camera pose
      std::vector<std::vector<std::vector<pcl::PointXYZI>>> img_pts_container(
          cam.height_, std::vector<std::vector<pcl::PointXYZI>>(cam.width_));
      ...
      const Eigen::Vector3d t_(extrinsic_params[3], extrinsic_params[4],
                               extrinsic_params[5]);
      const cv::Mat t_vec = (cv::Mat_<double>(3, 1) << t_(0), t_(1), t_(2));

      std::vector<cv::Point3f> pts_3d;
      std::vector<cv::Point2f> pts_2d;

      for (size_t i = 0; i < lidar_edge_clouds_3d->size(); ++i) {
        const pcl::PointXYZI& point_3d = lidar_edge_clouds_3d->points[i];
        const Eigen::Vector3d pt1(point_3d.x, point_3d.y, point_3d.z);
        const Eigen::Vector3d pt2(0, 0, 1);
        const Eigen::Vector3d pt =
            base_poses[a].q.inverse() * (pt1 - base_poses[a].t);

        if (cos_angle(q_ * pt + t_, pt2) > 0.8)  // FoV check
          pts_3d.emplace_back(cv::Point3f(pt(0), pt(1), pt(2)));
      }
      // project lidar 3d points to camera's image space.
      ...
      for (size_t i = 0; i < pts_2d.size(); ++i) {
        pcl::PointXYZ p;
        p.x = pts_2d[i].x;
        // inverse y's sign to match opencv image coordinate's convention.
        p.y = -pts_2d[i].y;
        p.z = 0;
        ...
        if (p.x > 0 && p.x < cam.width_ && pts_2d[i].y > 0 &&
          ...
          if (img_pts_container[pts_2d[i].y][pts_2d[i].x].empty()) {
      ...
      if (show_residual && a == 16) {
        const cv::Mat residual_img = getConnectImg(
            cam, dis_threshold, cam_edge_clouds_2d[a], line_edge_cloud_2d);
        const std::string img_name =
            "Residual between edge points from LiDAR and camera";
      ...
      const pcl::PointCloud<pcl::PointXYZ>::Ptr& tree_cloud_cam =
          cam_edge_clouds_2d[a];
      const pcl::PointCloud<pcl::PointXYZ>::Ptr& tree_cloud_lidar =
          line_edge_cloud_2d;
      const pcl::PointCloud<pcl::PointXYZ>::Ptr& search_cloud =
          line_edge_cloud_2d;
      ...
      // search_cloud is projected Lidar 2d edge points in image frame.
      for (size_t i = 0; i < search_cloud->points.size(); ++i) {
        const pcl::PointXYZ& searchPoint = search_cloud->points[i];
        ...
        if (kdtree_cam->nearestKSearch(searchPoint, K, pointIdxNKNSearch,
          ...
          for (int j = 0; j < K; j++) {
            const float distance = std::hypot(
                searchPoint.x - tree_cloud_cam->points[pointIdxNKNSearch[j]].x,
                searchPoint.y - tree_cloud_cam->points[pointIdxNKNSearch[j]].y);
          ...
          if (dis_check) {
            // A projected lidar 2d edge point in image frame.
            const cv::Point p_l_2d(search_cloud->points[i].x,
                                   -search_cloud->points[i].y);
            // The first point of the camera edge point neighbor.
            const cv::Point p_c_2d(
                tree_cloud_cam->points[pointIdxNKNSearch[0]].x,
            ...
            // Compute the direction of points_cam from line-fitting.
            // This is attached to p_l_2d.
            ...
            for (size_t i = 0; i < pointIdxNKNSearchLidar.size(); i++) {
              const Eigen::Vector2d p(
            ...
            // Compute the direction of points_lidar from line-fitting.
            // This is also attached to p_l_2d.
      ...
      for (size_t i = 0; i < lidar_2d_list.size(); i++) {
        const int y = lidar_2d_list[i].y;
        const int x = lidar_2d_list[i].x;
        const int pixel_points_size = img_pts_container[y][x].size();
        ...
        if (pixel_points_size > 0) {
          ...
          const float cos_direction_theta =
              pnp.direction.dot(pnp.direction_lidar);
          if (cos_direction_theta < cos_direction_theta_min_ &&
              cos_direction_theta > cos_direction_theta_max_) {
  ...
  cv::Mat getConnectImg(
    ...
    const pcl::PointCloud<pcl::PointXYZ>::Ptr& tree_cloud_cam = rgb_edge_cloud;

    for (size_t i = 0; i < depth_edge_cloud->points.size(); ++i) {
      // Inverse y's sign to match opencv image coordinate's convention.
      const cv::Point2d p2(depth_edge_cloud->points[i].x,
                           -depth_edge_cloud->points[i].y);
      if (p2.x > 0 && p2.x < cam.width_ && p2.y > 0 && p2.y < cam.height_) {
        const pcl::PointXYZ& p = depth_edge_cloud->points[i];
      ...
      if (kdtree_cam->nearestKSearch(searchPoint, K, pointIdxNKNSearch,
        ...
        for (int j = 0; j < K; j++) {
          ...
          if (distance < dis_threshold) {
            ...
            if ((line_count++ % 3) == 0) {
              // Draws the residual between LiDAR and camera edge points.
              cv::line(
                  connect_img,
    ...
    // The camera's edge points are displayed in blue.
    ...
    // The LiDAR's edge points are displayed in red.
  ...
  void colorCloud(
      const Vector6d& extrinsic_params, int density, const Camera& cam,
    ...
    const cv::Mat r_vec =
        (cv::Mat_<double>(3, 1) << rotation_vector3.angle() *
         ...
         rotation_vector3.angle() * rotation_vector3.axis().transpose()[1],
         rotation_vector3.angle() * rotation_vector3.axis().transpose()[2]);
    const cv::Mat t_vec = (cv::Mat_<double>(3, 1) << extrinsic_params[3],
                           extrinsic_params[4], extrinsic_params[5]);

    const Eigen::Quaterniond q_(rotation_vector3);
    const Eigen::Vector3d t_(extrinsic_params[3], extrinsic_params[4],
                             extrinsic_params[5]);

    for (size_t a = 0; a < rgb_imgs.size(); ++a) {
      std::vector<cv::Point3f> pts_3d;
      for (size_t b = 0; b < base_poses.size(); ++b) {
        for (size_t i = 0; i < lidar_clouds[b]->size(); i += density) {
          // Applies camera pose.
          const pcl::PointXYZI& point = lidar_clouds[b]->points[i];
          ...
          if (cos_angle(q_ * pt1 + t_, pt2) > 0.8) {
            ...
            if (depth > 2.5 && depth < 50) {
              pts_3d.emplace_back(pt1(0), pt1(1), pt1(2));
            }
          }
        }
      }
      ...
      const int image_rows = rgb_imgs[a].rows;
      const int image_cols = rgb_imgs[a].cols;
      pcl::PointCloud<pcl::PointXYZRGB>::Ptr color_cloud(
          new pcl::PointCloud<pcl::PointXYZRGB>);
      for (size_t i = 0; i < pts_2d.size(); ++i) {
        if (pts_2d[i].x > 1 && pts_2d[i].x < image_cols - 1 &&
            pts_2d[i].y > 1 && pts_2d[i].y < image_rows - 1) {
          const cv::Scalar color = rgb_imgs[a].at<cv::Vec3b>(pts_2d[i]);
