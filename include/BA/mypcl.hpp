...
std::vector<pose> read_pose(std::string path) {
  ...
  while (!file.eof()) {
    ...
    pose_vec.emplace_back(
        pose(Eigen::Quaterniond(w, x, y, z), Eigen::Vector3d(tx, ty, tz)));
    cnt++;
  }
  file.close();
  return pose_vec;
...
pcl::PointCloud<PointType>::Ptr append_cloud(
    pcl::PointCloud<PointType>::Ptr pc1, pcl::PointCloud<PointType> pc2) {
