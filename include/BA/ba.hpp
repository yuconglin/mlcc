...
class OCTO_TREE_NODE {
 public:
  ...
  ros::Publisher pub_center =
      nh.advertise<sensor_msgs::PointCloud2>("/child_center", 1000);
  ros::Publisher pub_voxel =
      nh.advertise<sensor_msgs::PointCloud2>("/child_voxel", 1000);
  ros::Publisher pub_cloud0 =
      nh.advertise<sensor_msgs::PointCloud2>("/child_cloud0", 1000);
  ros::Publisher pub_cloud1 =
      nh.advertise<sensor_msgs::PointCloud2>("/child_cloud1", 1000);
  ros::Publisher pub_cloud2 =
      nh.advertise<sensor_msgs::PointCloud2>("/child_cloud2", 1000);
  ros::Publisher pub_cloud3 =
      nh.advertise<sensor_msgs::PointCloud2>("/child_cloud3", 1000);
  ros::Publisher pub_direct =
      nh.advertise<visualization_msgs::MarkerArray>("/child_direct", 1000);
#endif
  ...
  bool judge_eigen(int layer) {
    ...
    const double eva0 = saes.eigenvalues()[0];
    const double sqr_eva0 = sqrt(eva0);
    ...
    for (int i = 0; i < 8; i++)
      if (covMats[i].N > MIN_PT) {
        ...
        if (fabs(child_direct.dot(direct)) > 0.98) num_qua++;
        num_all++;
      }
