...
class EXTRIN_OPTIMIZER {
 public:
  ...
  void push_voxel(std::vector<vector_vec3d*>& baseOriginPc,
    ...
    int refLidarPc = 0;
    for (int i = 0; i < pose_size; i++)
      if (!refOriginPc[i]->empty()) refLidarPc++;

    if (refLidarPc < 1)  // no pc from extrinsic
      return;

    if (refLidarPc == 1 &&
        baseLidarPc == 0)  // only 1 extrinsic pc and no base pc
      return;
  ...
  void calculate_HJ(vector_quad& poses, vector_vec3d& ts, vector_quad& refQs,
    ...
    for (int i = head; i < end; i++) {
      ...
      for (size_t j = 0; j < refpts_size; j++) {
        ...
        point_xis[j] =
            -wedge(vec_tran);  // poses[refwin_num[j]].toRotationMatrix()
        vec_tran = poses[refwin_num[j]] * vec_tran;
        pt_trans[j] =
            vec_tran + poses[refwin_num[j]] * refTs[0] + ts[refwin_num[j]];
      ...
      for (size_t j = 0; j < refpts_size; j++) {
        ...
        _jact.block<3, 1>(6 * 0, 0) -=
            point_xis[j] * poses[refwin_num[j]].toRotationMatrix().transpose() *
            vec_Jt;
      ...
      for (size_t j = 0; j < refpts_size; j++)  // for each point in voxel
      {
        ...
        for (size_t k = 0; k < refpts_size; k++) {
          ...
          _hess.block<3, 3>(rownum + 3, colnum + 3) +=
              ...
              poses[refwin_num[j]].toRotationMatrix();
  ...
  void evaluate_only_residual(vector_quad& poses_, vector_vec3d& ts_,
                              vector_quad& refposes_, vector_vec3d& refts_,
                              double& residual) {
    residual = 0;
    ...
    for (size_t i = 0; i < voxel_size; i++) {
      ...
      for (size_t j = 0; j < refWinNums[i]->size(); j++) {
        ...
        pt_trans =
            poses_[(*refWinNums[i])[j]] * pt_trans + ts_[(*refWinNums[i])[j]];
        new_A += pt_trans * pt_trans.transpose();
        new_center += pt_trans;
      }
 ...
 public:
  ...
  OCTO_TREE(int window_size, double eigen_limit)
    ...
    for (int i = 0; i < win_size; i++) {
      baseOriginPc.push_back(new vector_vec3d());
      baseTransPc.push_back(new vector_vec3d());
      refOriginPc.push_back(new vector_vec3d());
      refTransPc.push_back(new vector_vec3d());
    }
  }
        ...
        for (int i = 0; i < win_size; i++) {
          ...
          for (size_t j = 0; j < pt_size; j++) {
            ...
            for (size_t k = 0; k < 3; k++) {
              if ((*baseTransPc[i])[j][k] > voxel_center[k]) xyz[k] = 1;
            }
          ...
          for (size_t j = 0; j < pt_size; j++) {
            int xyz[3] = {0, 0, 0};
            for (size_t k = 0; k < 3; k++) {
              if ((*refTransPc[i])[j][k] > voxel_center[k]) xyz[k] = 1;
            }
            int leafnum = 4 * xyz[0] + 2 * xyz[1] + xyz[2];
            if (leaves[leafnum] == nullptr) {
              leaves[leafnum] = new OCTO_TREE(win_size, eigen_ratio);
